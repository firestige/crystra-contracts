...
| 字段 | 当前值 |
|---|---|
| 检查点 | C052：UI 45569e5 / DSH 82c4d88 完成 Task Browser 返回视图/精确条目恢复与共享键盘菜单；UI409+34单测、38浏览器，DSH231项通过。3082真实目录往返验证通过，继续 C053 Workflow Explorer；未发布新 RC |
...
| 工作路径 | 新 UI /tmp/crystra-ui-host-integration `45569e5`；新宿主 /tmp/crystra-dsh-t6 `82c4d88`（未发布）。原组合 /tmp/crystra-combination-stage 1593f589；/tmp/crystra-dsh-t6 产品候选 9a9777d、另有 test-only 742e1de；实际 ~/.dsh/profiles/web；台账 /Users/firestige/Projects/wsr-contracts |
...
| 下一条动作 | C053 按 v8 接 Workflow Explorer 精确版本目录及 Workflow 页面；随后接草案 value 验证与持续失效、真实数据端口；继续处理[新 UI 接入缺口](t8-ui-integration-gap.md)，再发布新 RC；原 UI 验收仅证明旧链路可运行。限流为独立复验项。公开 RC 安装入口已更新至 #277/#38，PR 保持 bot／非 Draft，等待正常合并 |
